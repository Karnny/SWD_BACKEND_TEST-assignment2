...
from apis.serializers.models import PersonnelModelSerializer, StudentSubjectsScoreModelSerializer
from apis.serializers.views import StudentSubjectsScoreViewSerializer
...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        credits_context = [{"id": 6, "credit": 1, "subject_id_list_that_using_this_credit": [3]},
                           {"id": 7, "credit": 2,
                               "subject_id_list_that_using_this_credit": [2, 4]},
        ...
        def get_creditID_by_subjectID(subject_id):
            for data in credits_mapping:
                if data["subject_id"] == subject_id:
                    return data["credit_id"]
            return None

        def get_credit_by_creditID(credit_id):
            for data in credits_context:
                if data["id"] == credit_id:
                    return data["credit"]
            return None
        ...
        serializer = StudentSubjectsScoreViewSerializer(data={
        ...
        subject_title = serializer.validated_data['subject_title']
        ...
        try:
            ...
            credit_id = get_creditID_by_subjectID(subject.pk)
            credit_value = get_credit_by_creditID(credit_id)

            score_instance, created = StudentSubjectsScore.objects.get_or_create(
                student=student, subjects=subject, credit=credit_value)
            ...
            if not created:
                return Response({
                    "student_details": PersonnelModelSerializer(student).data,
                    "subject_title": subject.title,
                    "subject_credit": score_instance.credit,
        ...
        except (Personnel.DoesNotExist, Subjects.DoesNotExist):
            return Response({'error': 'Invalid student or subject data.'}, status=status.HTTP_400_BAD_REQUEST)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        try:
            ...
            subject_info_list = []
            grade_point_sum = 0
            for score in student_subject_score:
                print(score)
                grade_point_sum += score.score
            ...
            grade_point_average_across_subjects = round(
                grade_point_sum / len(subject_info_list), 2)
            ...
            context_data = {
                ...
                "subject_detail": subject_info_list,
                "grade_point_average": grade_point_average_across_subjects
            }

        except (Personnel.DoesNotExist):
            return Response({'error': 'Invalid input, student ID does not exist.'}, status=status.HTTP_404_NOT_FOUND)
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        results pattern : in `results` variable below.
        ...
        try:
            ...
            personnel_details = Personnel.objects.filter(school_class__school=school).order_by(
                'personnel_type', 'school_class__class_order', 'first_name', 'last_name')

            resultss = []
            for idx, personnel in enumerate(personnel_details, start=1):
                role = get_role_name(personnel.personnel_type)
                class_order = personnel.school_class.class_order
                name = f"{personnel.first_name.capitalize()} {personnel.last_name.capitalize()}"
                resultss.append(
                    f"{idx}. school: {school.title}, role: {role}, class: {class_order}, name: {name}")

        except Schools.DoesNotExist:
            return Response({'error': 'Invalid input, school title does not exist.'}, status=status.HTTP_404_NOT_FOUND)
        
        except Exception as e:
            print(e)
            return Response({'error': 'Something went wrong.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        your_results = resultss

        return Response(your_results, status=status.HTTP_200_OK)
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        pattern: in `results` variable below.
        ...
        for school in schools:
            ...
            classes = Classes.objects.filter(
                school=school).order_by('class_order')
            ...
            for class_obj in classes:
                # Get the teacher for each class
                teacher = Personnel.objects.filter(
                    school_class=class_obj, personnel_type=0).first()

                class_info = {
                    f"class {class_obj.class_order}": {
                        f"Teacher: {teacher.first_name} {teacher.last_name}": []
                    }
                }

                head_of_room = Personnel.objects.filter(
                    school_class=class_obj, personnel_type=1).order_by('first_name', 'last_name')

                for head in head_of_room:
                    class_info[f"class {class_obj.class_order}"][f"Teacher: {teacher.first_name} {teacher.last_name}"].append({
                        "Head of the room": f"{head.first_name} {head.last_name}"
                    })

                # Get all students for each class
                students = Personnel.objects.filter(
                    school_class=class_obj, personnel_type=2).order_by('first_name', 'last_name')
        ...
        def build_hierarchy(data, parent_id=None):
            ...
            for item in data:
                if item['parent_id'] == parent_id:
                    ...
                    sub_hierarchy = build_hierarchy(data, parent_id=item['id'])
                    if sub_hierarchy:
                        item_data["sub"] = sub_hierarchy
